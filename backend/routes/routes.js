...
const editYourPet = require('../controllers/editYourPet');
...
router.put('/edit-pet',(req,res)=>{
  const {name, sex , age, image , quality , color , owner_id, breed , address, number, reason ,pet_id} = req.body;
  console.log('inside the add request of pet and the owner is : ', owner_id);
  ...
  console.log(typeof(image));
  petModal.findById(pet_id).then((result)=>{
    result.updateOne(({
      name : name,
    age: age,
    sex : sex,
    ...
    reason : reason
      ...
      console.log('updated the info ',ress);
      res.status(200).json({res : 'updated the info'});
    }).catch((err)=>{
      console.log('There was some error while updating ',err);
